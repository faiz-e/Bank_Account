# Current Account will be a type of Account
...
class Current(Account):
    ...
    def withdraw_amount(self, amount):
        if isinstance(amount, float) and amount > 0.0:  # if amount is a positive float number
            if self._amount > amount:       # if enough amount is present in account
                self._amount -= amount       # add that amount to account totals
                return self._amount
            else:
